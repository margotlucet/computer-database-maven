...
public class ComputerDAOImpl implements ComputerDAO {
	 ...
	 */
	@Override
	public long add(Computer c) throws SQLException{
		long id = 0;
		...
		try{
			...
			stmt = cn.prepareStatement("INSERT INTO computer (name, introduced, "
					+ "discontinued, company_id) VALUES (?, ?, ?, ?)",PreparedStatement.RETURN_GENERATED_KEYS);
	...
	public List<Computer> getResultList(ResultSet rs){
		...
		try {
			...
			LOGGER.error("Error occured while trying to get a result list");
			e.printStackTrace();
		}
		return list;
	}
	...
	public Computer getById(long id){
		...
		try {
			...
			stmt = cn.prepareStatement("SELECT computer.id, computer.name, introduced, discontinued, company_id, "
	...
	public void update(Computer c){
		...
		try {
			cn = DAOFactory.INSTANCE_DAO.getConnexion();
			stmt = cn.prepareStatement("UPDATE computer SET name=?, introduced=?, discontinued=?, "
					+ "company_id=? WHERE id=?;");
			stmt.setString(1, c.getName());
			if(c.getIntroduced()!=null)
				stmt.setTimestamp(2, new Timestamp(c.getIntroduced().getTime()));
	...
	public int getAmount() {
		...
		int number = 0;
		try{
			cn = DAOFactory.INSTANCE_DAO.getTl().get();
